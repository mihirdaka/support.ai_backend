...
export default class ApiUtility {
  ...
  static sanitizeUserWithAny(data : any) {
    const { password, isDeleted, ...basicUser } = data;
    return basicUser;
  }
  ...
  static sanitizeRawData(rawData: any,repository: string) {
    const data = JSON.parse(JSON.stringify(rawData)?.replaceAll(repository+'_', ''))
    return data;
  }
